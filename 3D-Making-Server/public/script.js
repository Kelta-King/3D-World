...
var g_hero = undefined;
var g_previous_state = undefined;
...
function VDSO_UI(name) {
    ...
    var screenBody = BABYLON.MeshBuilder.CreateBox(name, {width:16, height: 9, depth: 0.5, faceUV: faceUV}, scene);
    ...
    // screen.material = new BABYLON.StandardMaterial("planeMaterial", scene);
    // screen.material.diffuseTexture = videoTexture;
    // screen.material.roughness = 1;
    // screen.material.emissiveColor = new BABYLON.Color3.White();
...
var buildScene = async function (scene) {
    ...
    room.state.players.onAdd((player, sessionId) => {
        ...
        if (isCurrentPlayer) {
            ...
            g_hero.position.set(player.x, player.y, player.z);

            scene.registerBeforeRender(function () {
                let flag = false;
                if(g_previous_state == undefined)
                {
                    console.log("Once");
                    flag = true;
                    g_previous_state = {x: g_hero.position.x, y: g_hero.position.y, z: g_hero.position.z};
                }
                else
                {
                    flag = true;
                    var tempX = getMode(g_previous_state.x - g_hero.position.x);
                    var tempY = getMode(g_previous_state.y - g_hero.position.y);
                    var tempZ = getMode(g_previous_state.z - g_hero.position.z);
                    
                    if(tempX < 0.05 && tempY < 0.05 && tempZ < 0.05)
                    {
                        flag = false;
                    }
                    g_previous_state = {x: g_hero.position.x.toFixed(3), y: g_hero.position.y.toFixed(3), z: g_hero.position.z.toFixed(3)};
                }
            ...
            // Create Hero of other player
            var otherHero = createPlayer();

            // Set position of that player according to the message from server
            otherHero.position.set(player.x, player.y, player.z);
            
            // Set player entity and next postion object
            playerEntities[sessionId] = otherHero;
            playerNextPosition[sessionId] = otherHero.position.clone();

            // Register change handler for other player on the basis of position
            player.onChange(() => {
...
var createScene = function (engine) {
    ...
    var screen1 = VDSO_UI("screen1");

    screen1.position.x = 51.6;
    screen1.position.y = 13.25
    screen1.position.z = 51;
    screen1.scaling.x = 1.2;
    // screen1.scaling.y = 0.8;
    ...
    Positions for VDSOs
    Bank: 
    screen1.position.x = 40.8198819336906666;
    screen1.position.y = 50.25
    screen1.position.z = -31.13;
    screen1.rotation.y = Math.PI;
    screen1.scaling.x = 1.2;
    screen1.scaling.y = 1.2;
    

    Theatre:
    screen1.position.x = -31.13;
    screen1.position.y = 34.25
    screen1.position.z = -32.5;
    screen1.rotation.y = Math.PI;
    screen1.scaling.x = 1.5;
    screen1.scaling.y = 1.5;


    Game Parlour:
    screen1.position.x = -55.6;
    screen1.position.y = 23.25
    screen1.position.z = -3.5;
    ...
    Restaurant
    screen1.position.x = -31.6;
    screen1.position.y = 13.25
    screen1.position.z = 51;
    screen1.scaling.x = 0.8;
    screen1.scaling.y = 0.8;
    and
    screen1.position.x = -59.6;
    screen1.position.y = 13.25
    screen1.position.z = 51;
    screen1.scaling.x = 0.8;
    screen1.scaling.y = 0.8;
